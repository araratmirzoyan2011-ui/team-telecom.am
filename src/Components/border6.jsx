export function border6(inf1, inf2, inf3, sizeClass, bgImage, onClickHandler) {
  return (
    <div 
      className={`relative flex flex-col md:flex-row justify-between items-center rounded-3xl text-white shadow-lg hover:shadow-2xl transition-all duration-300 ease-in-out cursor-pointer overflow-hidden group border border-white/10 p-5 md:p-6 h-auto md:h-[300px] ${sizeClass}`}
    ...
    >
      ...
      <div className="w-full md:w-1/2 h-full flex flex-col items-start justify-between opacity-0 translate-y-5 animate-[fadeInUp_1s_ease-out_forwards] z-10 pr-2">
        <div>
          <h1 className="text-2xl md:text-[32px] font-bold tracking-wide line-clamp-2 text-white group-hover:text-red-400 transition-colors duration-200">
            {inf1}
          </h1>
        ...
        <div className="flex items-center text-sm sm:text-base font-medium text-gray-200 group-hover:text-white transition-colors mt-4 md:mt-0">
          <span className="border-b border-transparent group-hover:border-red-500 transition-all">{inf3}</span>
