...
// Local default data (որպեսզի ինֆորմացիան չկորչի)
const defaultArr1 = [
  ["https://www.telecomarmenia.am/images/block_with_icons_icons/1/17537347165186.png", "Visit Team e-Shop and choose smartphone any smartphone/smartphones and add to basket."],
...
const defaultArr1acba = [
  ["https://www.telecomarmenia.am/images/block_with_icons_icons/1/17617368528774.png", "Visit Team e-Shop and choose smartphone any smartphone/smartphones and add to basket."],
...
const defaultArr2 = [
  ["https://www.telecomarmenia.am/images/block_with_icons_icons/1/16510387043795.png", "Visit Team e-Shop and choose smartphone and/or nice number and add to basket."],
...
function OnlineCredit() {
  ...
  const [arr1acba, setArr1acba] = useState(defaultArr1acba);
  const [arr2, setArr2] = useState(defaultArr2);
  const [faqLeft, setFaqLeft] = useState(defaultFaqLeft);
  const [faqRight, setFaqRight] = useState(defaultFaqRight);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, "onlineCredit"));
        if (!querySnapshot.empty) {
          const data = querySnapshot.docs[0].data();
          if (data.arr1 && data.arr1.length > 0) setArr1(data.arr1);
          if (data.arr1acba && data.arr1acba.length > 0) setArr1acba(data.arr1acba);
          if (data.arr2 && data.arr2.length > 0) setArr2(data.arr2);
          if (data.faqLeft && data.faqLeft.length > 0) setFaqLeft(data.faqLeft);
          if (data.faqRight && data.faqRight.length > 0) setFaqRight(data.faqRight);
        }
      } catch (error) {
  ...
  return (
    <>
      ...
      <div className='mt-[100px] w-full min-h-[500px] flex items-center justify-center bg-[#083f58]'>
        <N2
